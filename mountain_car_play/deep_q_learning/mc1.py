...
for episode in range(NUM_EPISODES):
    ...
    while not done:
        ...
        if step_counter % LEARN_AFTER_STEPS == 0:
            ...
            with tf.GradientTape() as tape:
                ...
                num_batches = tf.range(0, limit=BATCH_SIZE)
                indices = tf.stack((num_batches, actions), axis=1)
    ...
    pd.DataFrame(metric).to_csv("metrics2.csv", index=False)
    EPSILON /= EPSILON_DECAY
env.close()
q_net.save("dqnet2.keras")
