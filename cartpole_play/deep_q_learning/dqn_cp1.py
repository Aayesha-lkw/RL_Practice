...
while i<20 and not done:
    ...
    state, rewards, done, _, _ = env.step(policy(state).numpy())
    i += 1
...
for episode in range(EPISODE_NUM):
    ...
    #print("Episode: ", episode, "Episode Length: ", episode_length, "Reward: ", total_rewards, "Epsilon: ", EPSILON, "Avg Q: ", avg_q)
    pd.DataFrame(metric).to_csv("metric.csv", index=False)
    EPSILON /= EPSILON_DECAY
env.close()
q_net.save("dqn_q_net1.keras")
